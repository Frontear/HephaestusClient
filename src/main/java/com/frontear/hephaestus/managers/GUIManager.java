...
public class GUIManager extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawRect(x_pos, 2, 80, mc.fontRendererObj.FONT_HEIGHT * 2, new Color(128, 128, 128).getRGB());
    ...
    public void initGui() {
        for (int i = 0; i < getModuleList().size(); i++) {
            if (getModuleList().get(i).name != "ClickGui") {
                buttonList.add(new GuiButton(i, x_pos, (12 + mc.fontRendererObj.FONT_HEIGHT * 2) + (20 * i), 119, mc.fontRendererObj.FONT_HEIGHT * 2, getModuleList().get(i).name));
            }
        }
    }
    ...
    private ArrayList<Module> getModuleList() {
        return Hephaestus.client.moduleManager.moduleList;
    }
}
